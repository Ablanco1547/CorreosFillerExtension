...
let tabla = document.querySelector("#tablaSelect")




const buscarCanton = (provincias, barraCanton) =>{

    for(let i = 0; provincias.length > i; i++){
        let cantonesProvincia = provincias[i].cantones;



        for(let j = 0; cantonesProvincia.length > j; j++){
            if(cantonesProvincia[j].nombreCanton.includes(barraCanton.value)){
                let tablaselect = document.querySelector("#tablaSelect")
                    let contador = 0;
                    let fila= tablaselect.insertRow(contador);
                    let provincia = fila.insertCell(0);
                    let canton = fila.insertCell(1);
                    provincia.innerHTML = provincias[i].nombreProvincia;
                    canton.innerHTML = cantonesProvincia[j].nombreCanton;
            }
        }
    }
...
const buscarDistrito = (provincias, barraDistrito) =>{
    ...
    for(let i = 0; provincias.length > i; i++){
        ...
        for(let j = 0; cantonesProvincia.length > j; j++){
            ...
            let distritosCanton = cantonesProvincia[j].distritos;

            for(let k = 0; distritosCanton.length > k; k++){

                if(distritosCanton[k].nombreDistrito.includes(barraDistrito.value)){
                    let tablaselect = document.querySelector("#tablaSelect")
                    let contador = 0;
                    let fila= tablaselect.insertRow(contador);
                    let provincia = fila.insertCell(0);
                    let canton = fila.insertCell(1);
                    let distrito = fila.insertCell(2);
                    provincia.innerHTML = provincias[i].nombreProvincia;
                    canton.innerHTML = cantonesProvincia[j].nombreCanton;
                    distrito.innerHTML = distritosCanton[k].nombreDistrito;
...
const buscarSelectProvincias = ( mapaProvincias, selectProvinciaValue)=>{
    let largoProvincias = mapaProvincias.provincias.length
    let provinciaAComparar = mapaProvincias.provincias;
    
    for (i = 0; largoProvincias > i; i++) {

        if( provinciaAComparar[i].nombreProvincia == selectProvinciaValue){
            return provinciaAComparar[i];
        }
    }
}

const buscarSelectCantones = (selectProvincia, selectCantonValue) =>{
    let largoCantones = selectProvincia.cantones.length

    for(let i = 0; largoCantones > i; i++){
        if(selectProvincia.cantones[i].nombreCanton == selectCantonValue){
            return selectProvincia.cantones[i];
        }
    }

}

const llenarCantones = (provinciaSeleccionada)=>{

    const cantones = provinciaSeleccionada.cantones

    vaciarOpciones(selectCanton);

    let opcionDefaultCanton = document.createElement("option");
    opcionDefaultCanton.text = "Seleccione el canton"
    selectCanton.add(opcionDefaultCanton)

    cantones.forEach(canton => {
        let nuevaOpcionCanton = document.createElement("option");
        nuevaOpcionCanton.text = canton.nombreCanton;
        nuevaOpcionCanton.value = canton.nombreCanton;
        selectCanton.add(nuevaOpcionCanton);
    });

}

const llenarDistritos = (cantonSeleccionado)=>{

    //console.log(cantonSeleccionado + " este")
    const distritos = cantonSeleccionado.distritos

    vaciarOpciones(selectDistrito);

    let opcionDefaultDistrito = document.createElement("option");
    opcionDefaultDistrito.text = "Seleccione el canton"
    selectDistrito.add(opcionDefaultDistrito)

    distritos.forEach(distrito =>{
        let nuevaOpcionDistrito = document.createElement("option");
        nuevaOpcionDistrito.text = distrito.nombreDistrito;
        nuevaOpcionDistrito.value = distrito.nombreDistrito;
        selectDistrito.add(nuevaOpcionDistrito);
    })
}


 
let vaciarOpciones = (select) =>{
    let todasOpciones = select.querySelectorAll("option")
...
const llenarTablaSelect = (provinciaSeleccionada, cantonSeleccionado, distritoSeleccionado) =>{
    let tablaselect = document.querySelector("#tablaSelect")
    let contador = 0;
    let fila= tablaselect.insertRow(contador);
    let provincia = fila.insertCell(0);
    let canton = fila.insertCell(1);
    let distrito = fila.insertCell(2);
    provincia.innerHTML = provinciaSeleccionada;
    canton.innerHTML = cantonSeleccionado;
    distrito.innerHTML = distritoSeleccionado;
}

let revisarVacio = (campo) =>{
...
barraCanton.addEventListener('input',(buscarCantones) => {
    
    limpiarTabla(tabla)
    buscarCanton(mapaProvincias.provincias,barraCanton)
    revisarVacio(barraCanton)

    
});

barraDistrito.addEventListener('input',(buscarDistritos) => {
    limpiarTabla(tabla)
    buscarDistrito(mapaProvincias.provincias,barraDistrito);
    revisarVacio(barraDistrito)
    
});


selectProvincia.addEventListener("change", (popularC) => {
    
    provinciaSeleccionada = buscarSelectProvincias(mapaProvincias, selectProvincia.value);
    llenarCantones(provinciaSeleccionada);
});


selectCanton.addEventListener("change", (popularD) =>{
    cantonSeleccionado = buscarSelectCantones(provinciaSeleccionada, selectCanton.value)
    llenarDistritos(cantonSeleccionado)
})

selectDistrito.addEventListener("change",(llenar =>{

    limpiarTabla(tabla)
    distritoSeleccionado = selectDistrito.value;
    llenarTablaSelect(provinciaSeleccionada.nombreProvincia, cantonSeleccionado.nombreCanton, distritoSeleccionado)
}))